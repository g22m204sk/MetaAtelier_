...
namespace MakeCode
{
    class Program
    {
        static Server server = new Server("dddd@xs238699.xsrv.jp", "11111111", "xs238699.xsrv.jp");
        const int start = 86431;
        const int end = 100000;
        const string base_ = "contents/MetaAteliersPICTURE/scl/";
        static string xw = "";
        static WebClient client = new WebClient(); 
        static void Main(string[] args)
        {
            string[] cd = File.ReadAllLines(@"C:\Users\sudolab\3D Objects\MetaAteliers PICTURE\SerialCode.txt");
            for (int v = start; v <= end; v++)
            { 
                server.webClient.UploadString(server.accessUrlBase + base_ + cd[v].Substring(1,8) + ".txt", v.ToString().PadLeft(6, '0'));
                Console.WriteLine(v);
            }
        }
        ...
        static string CMKC
        {
            ...
            {
                string w = "";
                do w = mkc; while (Exist(w));
                return w;
            }
        }

        static string mkc { get => Guid.NewGuid().ToString().Replace("-", "").ToUpper().Substring(0, 8);  }

        static bool Exist(string cx)
        {
            try
            {
                client.DownloadString("http://xs238699.xsrv.jp/contents/MetaAteliersPICTURE/scl/" + cx + ".txt");
                return true;
            }
...
namespace ss
{
    ...
    public class Server
    {
        ...
        public Server(string useraddress, string password, string serveraddress)
        {
            ...
            this.accessUrlBase = "ftp://" + useraddress + "/%2f" + serveraddress + "/public_html/";
        ...
        public void FileUpload(string local, string url, TaskEvent finished = null, ErrorEvent error = null)
        {
            ...
            catch (WebException e)
            {
                try
                {
                    var ii = (FtpWebResponse)e.Response;
                    if (e != null)
                    {
                        if (ii != null)
                        {
                            error?.Invoke(ii.StatusDescription);
                        }
                    }
                }
                catch (Exception ex)
